...
​	信号量：进程注册一个约定的信号量处理函数来响应信号，这里的信号量与上面提到的信号相似，只是这里是实现进程之间通信的。
...
​	管理进程的使用周期，创建进程、退出进程、父进程与子进程相互访问等等。还需要注意fork时为了节约时间而引入的写时复用技术。



#### 僵死进程

​	子进程退出时需要告知父进程处理，否则会变成僵死进程。有两种方法：

1. 父进程调用wait4等待子进程退出
2. 父进程已经退出，子进程还没有退出，此时内核会将子进程的父进程设置为init进程，由init进程使用wait4回收僵死进程



#### 进程组和登录会话

​	对于一些一系列命令组成的"作业"，Unix引入了进程组的概念。例如作业命令`ls | sort |more`是由进程组实现。

​	登录会话是进程与终端的交流窗口，一个登录会话可以让多个进程组处于活跃状态，但只有一个进程组处于前台，其他进程组处于后台，前台进程组可以访问终端。如果后台进程组访问终端，它将会收到SIGTTIN和SIGTTOU信号，一般用bg和fg把进程组放在后台或前台。



#### 虚拟内存

​	Unix内核在应用程序的内存请求和硬件内存管理单元（MMU）之间提供了一种逻辑层，称为虚拟内存。虚拟内存的用途和优点有：

1. 多个进程可以并发执行
2. 应用程序所需的内存大于实际可用物理内存时也可以运行
3. 程序只有部分代码装入内存时进程可以执行它
4. 允许每个进程访问可用物理内存的子集
5. 进程可以共享库函数或程序的一个单独内存抽象
6. 程序可重定位。可以把程序放到物理内存的任何地方
7. 不需要关注物理内存的结构

​	虚拟地址由内核和MMU协同定位：MMU将RAM划分为4K或8K的页，并引入一组或多组页表来指定虚拟地址和物理地址的对应关系，内核对虚拟地址的请求就可以通过分配一组非连续的物理地址页来实现。

​	虚拟内存需要注意内存碎片，通常内核要求使用物理内存上连续的内存区域，如果碎片太多，而没有连续的内存区域，那么内存请求会失败。



#### 随机访问存储器（RAM）的使用

​	Unix内核将RAM划分为两部分：1. 存放内核映像（内核代码和内核静态数据结构） 2. 由虚拟内存子系统处理。

​	其中虚拟内存子系统主要的工作有：

1. 满足内核对缓冲区、描述符和动态内核数据结构的请求
2. 满足进程对内存区的请求
3. 为了提高读写磁盘效率而划分的缓冲区

​	内存是有限的，当内存达到上限阈值时可以调用页框回收算法来释放内存，但是需要释放哪一部分的内存，需要应用程序根据实际情况决定。

​	

#### 内核内存分配器（KMA）

​	内核内存分配器是一个子系统，用来满足所有部分对内存的请求。

​	KMA应该具有以下特点：

1. 速度快。因为KMA需要处理所有内核子系统（包括中断处理程序）的调用
2. 把内存浪费减少到最少
3. 高效减轻内存碎片问题
4. 能与其他内存管理子系统合作，以便借用和释放页框

​	Linux的KMA在伙伴系统上采用了Slab分配算法



#### 进程虚拟地址空间处理

​	进程虚拟地址空间包括了进程可以引用的所有虚拟内存地址。内核使用一组内存区描述符来指示进程的虚拟地址空间，内核分配给进程的虚拟地址空间由以下内存区组成：
...
​	磁盘读写速度相比RAM访问速度慢很多倍，为了提供使用效率，Unix实现的策略是：尽可能推迟写磁盘的时间，从磁盘读入内存的数据即使任何进程都不再使用它们，它们也继续留在RAM中。而进程在访问磁盘时，会首先检查访问的数据是否在内存中，如果在就直接使用内存的数据而不需要重新读写磁盘。

​	对于内存中修改的数据，内核会将数据标记为"脏"，然后周期性的调用sync将脏缓存写入磁盘。



#### 设备驱动程序

​	内核通过设备驱动程序与I/O设备交互，设备驱动程序的优点：

1. 可以把特定设备的代码封装在特定的模块中
2. 在不了解内核代码，而只知道接口规范的情况下，增加新的设备
3. 内核以统一的方式对待所有的设备，并且使用相同的接口访问这些设备
4. 可以把内核写成模块，并动态把它们装进内核而不需要重新启动系统，不需要时也可以动态卸载模块。这样既灵活方便，又能减少存储在RAM中的内核映像的大小
